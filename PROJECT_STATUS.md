...
4.1 Regla de uso de NULL (OBLIGATORIA)

El uso de NULL en el modelo de datos está permitido solo bajo condiciones explícitas y controladas.
Se permite NULL únicamente cuando:
El atributo representa un estado futuro válido

Ejemplos:
fecha_cierre mientras el caso no está cerrado
fecha_baja mientras el registro está activo
La información no existe legítimamente en el momento del registro

Ejemplos:
reporte.id_area cuando el lugar del hecho no es conocido o no aplica
datos aún no determinados al momento del reporte inicial
No se permite NULL:
para evadir diseño del modelo
para evitar validaciones de negocio
en atributos estructurales obligatorios (ej: id_colegio, estados, descripciones base)
Esta regla es transversal al proyecto y se aplica a todas las tablas nuevas y existentes.
